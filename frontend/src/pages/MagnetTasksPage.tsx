...
export default function MagnetTasksPage() {
  ...
  const submitWhitelistBatch = async () => {
    ...
    try {
      const response = await api.post<WhitelistBatchSubmitResponse>('/magnet-tasks/whitelist-batch/submit', {
        tree_import_id: selectedTreeImportId,
        keyword_entry_ids: selectedWhitelistKeywordIds,
        per_keyword_limit: perKeywordLimit,
        total_limit: totalLimit,
      ...
      <Card className="soft-card" style={{ marginBottom: 16 }} title="白名单批处理">
        <Space direction="vertical" size={14} style={{ width: '100%' }}>
          <Alert
            ...
            showIcon
            message="适合日常增量跑"
            description="系统会按 active whitelist 批量检索外部库，跨关键词合并候选，再结合本地目录树去重。提交时自动创建 115 目录 /已整理/关键词/磁力名称，并按 submit_limit 控制单次提交数量。"
          />
          ...
          <Space size={16} wrap>
            ...
            <Statistic title="本次预览候选" value={batchStats?.selected_candidates ?? batchPreview.length} />
      ...
      <Card className="soft-card" title={`白名单批处理预览（${batchPreview.length}）`} style={{ marginBottom: 16 }}>
        {batchStats ? (
          <Alert
            ...
            description={`当前会按 submit_limit=${submitLimit} 控制实际提交数。若白名单或本地目录树持续增长，可以反复执行这一入口，重复内容会先被本地目录树去重和任务表拦截。`}
          />
        ) : (
          <Alert
            style={{ marginBottom: 16 }}
