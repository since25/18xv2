...
@dataclass(slots=True)
class WhitelistBatchPreviewItem:
...
@dataclass(slots=True)
class WhitelistBatchPreviewRun:
    whitelist_entries: list[KeywordEntry]
    preview_items: list[WhitelistBatchPreviewItem]
...
class MagnetDownloadService:
    ...
    def preview_whitelist_batch(
        ...
        *,
        ...
        total_limit: int = 100,
    ) -> WhitelistBatchPreviewRun:
        whitelist_entries = self._load_whitelist_entries(keyword_entry_ids)
        directory_names = OrganizeTaskService._build_keyword_directory_names(whitelist_entries)
        merged: dict[tuple[int, str], WhitelistBatchPreviewItem] = {}
        duplicate_cache: dict[tuple[int, str], DuplicateCheckResult] = {}
        scanned_keyword_count = 0

        for entry in whitelist_entries:
            scanned_keyword_count += 1
            candidates = self.build_candidates_for_keyword_entry(keyword_entry=entry, limit=per_keyword_limit)
            for candidate in candidates:
                dedupe_key = (candidate.source_tid, candidate.source_magnet)
                duplicate_result = duplicate_cache.get(dedupe_key)
                if duplicate_result is None:
                    duplicate_input = DuplicateCheckItemRequest(
                        source_tid=candidate.source_tid,
                        source_title=candidate.source_title,
                        source_magnet=candidate.source_magnet,
                ...
                preview_item = WhitelistBatchPreviewItem(
                    item=MagnetTaskCreateItem(
                        source_tid=candidate.source_tid,
                        source_title=candidate.source_title,
                        source_magnet=candidate.source_magnet,
                ...
                if existing is None or preview_item.item.match_score > existing.item.match_score:
                    merged[dedupe_key] = preview_item

            if scanned_keyword_count % 50 == 0:
                logger.info(
                    "Whitelist preview progress import_id=%s scanned=%s/%s merged=%s",
                    tree_import_id,
        ...
        selected = sorted(
            ...
            key=lambda current: (
                ...
                current.item.source_tid,
            ),
        )
        preview_items = selected[:total_limit]
        logger.info(
    ...
    def submit_whitelist_batch(
        ...
        *,
        ...
        submit_limit: int = 20,
        force_submit: bool = False,
    ) -> tuple[WhitelistBatchPreviewRun, list[MagnetDownloadTask]]:
        preview_run = self.preview_whitelist_batch(
            tree_import_id=tree_import_id,
            keyword_entry_ids=keyword_entry_ids,
            per_keyword_limit=per_keyword_limit,
            total_limit=total_limit,
        )
        selected_items = preview_run.preview_items[:submit_limit]
        tasks = self.create_and_submit_tasks(
            items=[item.item for item in selected_items],
            force_submit=force_submit,
            tree_import_id=tree_import_id,
        )
        return preview_run, tasks
